...
def load_and_process_data(scenario):
    """
    Load and aggregate environmental data for the future scenario.
    For a future run, we assume the following file structure within MAIN_DIR:
      - PV_FUTURE
      - WHEAT_FUTURE
    """
...
def train_rl_model(
    ...
    Train a PPO model with early stopping:
      - stop if no mean‐reward improvement ≥ min_delta for `patience` evals
    ...
    # 2️⃣ Build a separate eval‐env (same params as train env)
    eval_env = MesaEnv(
    ...
    eval_cb = EvalCallback(
        ...
        eval_freq=eval_freq,
        best_model_save_path=None,  # we only care about early stop, not best‐model saving
        verbose=1,
    )
...
def main():
    ...
    if model_params.get("period", "").lower() != "future":
        logging.error("This script is intended for future scenario runs only.")
        return
    
    scenario = model_params.get("scenario", None)
    if scenario is None:
        logging.error("Scenario number not found in the input JSON.")
        return
    scenario_str = f"RCP{scenario}"
    ...
    if train_model:
        ...
        model_path = os.path.join(scenario_dir, f"{scenario_str}_ppo_mesa_model.zip")
        try:
