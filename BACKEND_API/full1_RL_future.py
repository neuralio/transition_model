...
Description:
    ...
    using a Mesa-based environment. It reads configuration (including the RCP scenario and PECS 
    parameters) from an input JSON file, aggregates environmental data from a fixed directory structure, 
    and then either trains a PPO model (if --train_model is set) or runs simulation episodes using a pre-trained model.
    
Directory structure:
    MAIN_DIR = "/app/INPUT_RL"
    INPUT_JSON_PATH = "/app/input_mongo_full1_future.json"
    BASE_DIR = os.path.dirname(MAIN_DIR)
    RESULTS_DIR = os.path.join(BASE_DIR, "RESULTS_RL", "FULL1_RL")
    
Usage:
    python full1_RL_future.py [--train_model] [--num_episodes NUM]
...
def create_aggregated_data(scenario, main_dir, output_dir):
    # For future data, define subdirectories accordingly.

    pv_suitability_file = os.path.join(MAIN_DIR, "PV_FUTURE", f"RCP{scenario}_PV_SUITABILITY_PREDICTIONS_UPDATED.csv")
    pv_yield_file       = os.path.join(MAIN_DIR, "PV_FUTURE", f"RCP{scenario}_PV_YIELD.csv")
    cs_wheat_file       = os.path.join(MAIN_DIR, "WHEAT_FUTURE", f"RCP{scenario}_LUSA_PREDICTIONS.csv")
    cy_wheat_file       = os.path.join(MAIN_DIR, "WHEAT_FUTURE", f"AquaCrop_Results_RCP{scenario}.csv")

    try:
        PV_ = pd.read_csv(pv_suitability_file, sep=',')
        PV_ = PV_.rename(columns={"score": "pv_suitability"})
        PV_ = PV_[(PV_["year"] >= 2021) & (PV_["year"] <= 2030)]
        logging.info(f"Loaded PV Suitability for RCP {scenario}")
    except Exception as e:
    ...
    try:
        ...
        logging.info(f"Loaded Crop Suitability for Wheat for RCP {scenario}")
    except Exception as e:
    ...
    try:
        ...
        logging.info(f"Loaded PV Yield for RCP {scenario}")
    except Exception as e:
    ...
    try:
        ...
        logging.info(f"Processed Crop Yield for Wheat for RCP {scenario}")
    except Exception as e:
    ...
    try:
        ...
        logging.info("Merged environmental dataset successfully.")
    except Exception as e:
...
# --------------------------
def main():
    parser = argparse.ArgumentParser(description="Unified RL training and simulation for full1_rl future.")
    ...
    logging.info("Starting full1_RL_future run.")
    ...
    logging.info("full1_RL_future run completed.")
