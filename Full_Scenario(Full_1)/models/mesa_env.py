...
class MesaEnv(gym.Env):
    def __init__(self, env_data, max_steps, pecs_params, width=5, height=5, scenario_params=None):
        ...
        self.model = LandUseModel(
            ...
            subsidy=self.scenario_params.get("subsidy", 0.0) if self.scenario_params else 0.0,
            loan_interest_rate=self.scenario_params.get("loan_interest_rate", 0.05) if self.scenario_params else 0.05,
            tax_incentive_rate=self.scenario_params.get("tax_incentive_rate", 0.0) if self.scenario_params else 0.0,
            social_prestige_weight=self.scenario_params.get("social_prestige_weight", 0.0) if self.scenario_params else 0.0
        )
