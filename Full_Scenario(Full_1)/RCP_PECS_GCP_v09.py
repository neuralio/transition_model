...
class MesaEnv(gym.Env):
    def __init__(self, env_data, max_steps, pecs_params, width=5, height=5, scenario_params=None):
        ...
        # Store pecs_params and scenario_params as instance variables
        ...
        self.model = LandUseModel(
            ...
            subsidy=self.scenario_params["subsidy"],
            loan_interest_rate=self.scenario_params["loan_interest_rate"],
            tax_incentive_rate=self.scenario_params["tax_incentive_rate"],
            social_prestige_weight=self.scenario_params["social_prestige_weight"]
        )
    ...
    def reset(self, *, seed=None, options=None):
        ...
        """
        self.current_step = 0
        # Reinitialize the model with pecs_params, new parameters, and seed
        self.model = LandUseModel(
            self.env_data,
            width=self.width,
            height=self.height,
            ...
            return np.array([
                {
                    ...
                    "loan_interest_rate": agent.loan_interest_rate,
                    "tax_incentive_rate": agent.tax_incentive_rate,
                    "social_prestige_weight": agent.social_prestige_weight,
...
if __name__ == "__main__":
    ...
    # Argument parsing with PECS parameters and new scenario parameters
    ...
    # New parameters for the scenarios
    parser.add_argument("--subsidy", type=float, default=0.0, help="Subsidy for PV installations (default: 0.0).")
    parser.add_argument("--loan_interest_rate", type=float, default=0.05, help="Loan interest rate for PV installations (default: 0.05).")
    parser.add_argument("--tax_incentive_rate", type=float, default=0.0, help="Tax incentive rate for renewable energy production (default: 0.0).")
    parser.add_argument("--social_prestige_weight", type=float, default=0.0, help="Weight of social prestige for early adopters (default: 0.0).")
    ...
    env = MesaEnv(
        ...
        width=width,
        height=height,
        scenario_params=scenario_params  # Pass scenario parameters
    )
...
if train_model:
    ...
    # Log PECS parameters and scenario-specific parameters
    ...
    # Save scenario-specific parameters to a JSON file
    scenario_params = {
        "subsidy": subsidy,
